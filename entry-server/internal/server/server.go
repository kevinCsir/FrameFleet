...
func New(registry *service.WorkerRegistry, heartbeatConfig HeartbeatConfig, splitPolicy protocol.SplitPolicy, processingPolicy protocol.ProcessingPolicy, appLogger *logger.Logger) *Server {
	if os.Getenv(gin.EnvGinMode) == "" {
		gin.SetMode(gin.ReleaseMode)
	}

	jobManager := service.NewJobManager(registry, processingPolicy, appLogger)
	engine := gin.New()
	engine.Use(requestLogger(appLogger), gin.Recovery())
	registerRoutes(engine, registry, jobManager, splitPolicy, processingPolicy, appLogger)
...
func (s *Server) ProcessingPolicy() protocol.ProcessingPolicy {
